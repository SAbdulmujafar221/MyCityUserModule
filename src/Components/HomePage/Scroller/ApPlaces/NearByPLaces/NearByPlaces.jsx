...
const MapComponent = () => {
    ...
    useEffect(() => {
        ...
        const fetchGeoJson = fetch('https://nominatim.openstreetmap.org/search?q=Andhra+Pradesh,India&format=geojson&polygon_geojson=1')
            .then(response => {
            ...
            .then(data => {
                ...
                setGeoJsonData({
                    andhraPradesh: { type: "FeatureCollection", features: andhraPradeshData },
    ...
    // Ensure places is always an array before using map()
    ...
    return (
        <div className="map-container">
            ...
            >
                ...
                />
                {geoJsonData?.andhraPradesh && (
                    <GeoJSON
                        data={geoJsonData.andhraPradesh}
                        style={{ color: 'red', weight: 2, dashArray: '4, 4', fillOpacity: 0 }}
                    />
                )}
                {geoJsonData?.otherStates && (
                    <GeoJSON
                        data={geoJsonData.otherStates}
                        style={{ color: 'grey', weight: 1, fillOpacity: 0 }}
                    />
                )}
                {filteredPlaces.map(place => (
                    <Marker
                        ...
                        icon={createIcon(categories.find(cat => cat.name === place.category)?.icon)}
                    >
                        <Popup>
